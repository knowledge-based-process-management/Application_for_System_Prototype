...
criteriaControl.loadTR4 = function() {
    ...
    // Add the buttons for calculate actual value
    ...
    // End "Add the buttons for calculate actual value"
    ...
    $('#Verification').find(".buttons").each(function(index, element) {
        var kpiValue= $(element).children("input").val();
        var item = $(element).closest(".item.fixed-size-item");
        var api = DnaAdapter_config.dna_server_url + "/function/"+apis[kpiValue];
        console.log("Querying " + kpiValue + "...");
        $.ajax({
            "url": encodeURI(api),
            "method": "GET",
